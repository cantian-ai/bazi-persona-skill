...
export function normalizeDateInput(rawDate: string): string {
...
function formatIso(date: string, time: string): string {
...
function resolveLongitude(location: string, manual?: number): number | undefined {
...
function applyDayRollover(date: string, time: string, dayRolloverHour: number): { date: string; time: string } {
...
function applyTrueSolarTime(time: string, longitude?: number): { time: string; correctionMinutes?: number } {
...
function normalizePillarValue(value: unknown): string | undefined {
  if (!value) {
    return undefined;
  }
...
function parsePillarsFromMarkdown(markdown: string): Pillars {
  const pillars: Pillars = {};
  PILLAR_PATTERN.lastIndex = 0;
  let match: RegExpExecArray | null = PILLAR_PATTERN.exec(markdown);
  while (match) {
...
function parseDayMasterFromMarkdown(markdown: string): string | undefined {
  const line =
    /(?:日主|日元)\s*[:：]\s*([甲乙丙丁戊己庚辛壬癸\w]+)/.exec(markdown) ??
    /\|\s*(?:日主|日元)\s*\|\s*([甲乙丙丁戊己庚辛壬癸\w]+)/.exec(markdown);
  return line?.[1];
}

function pruneHourSensitive(node: unknown): unknown {
  ...
  for (const [key, value] of Object.entries(record)) {
    if (
      /(hour|shi|hourPillar|timePillar|时柱|时支|时干|时宫)/i.test(key)
    ) {
      continue;
    }
...
function stripHourLines(markdown: string): string {
  let text = markdown;
  text = text.replace(/\n###\s*时柱[\s\S]*?(?=\n###|\n##|$)/g, "\n");
  text = text.replace(/\n##\s*刑冲合会[\s\S]*?(?=\n##|$)/g, "\n");
...
function sanitizeRawBaziForMissingTime(raw: unknown): unknown {
  const pruned = pruneHourSensitive(raw);
  if (!pruned || typeof pruned !== "object" || Array.isArray(pruned)) {
    return pruned;
  }
  const record = { ...(pruned as UnknownRecord) };
...
function ensureStructuredPillars(raw: unknown, markdown: string): Pillars {
  const fromMarkdown = parsePillarsFromMarkdown(markdown);

  const yearFromRaw = normalizePillarValue(
    deepFindByKeys(raw, ["yearPillar", "year", "nianZhu", "年柱"]),
  );
  const monthFromRaw = normalizePillarValue(
    deepFindByKeys(raw, ["monthPillar", "month", "yueZhu", "月柱"]),
  );
  const dayFromRaw = normalizePillarValue(
    deepFindByKeys(raw, ["dayPillar", "day", "riZhu", "日柱"]),
  );
  const hourFromRaw = normalizePillarValue(
    deepFindByKeys(raw, ["hourPillar", "hour", "shiZhu", "时柱"]),
  );
...
function normalizeFiveElements(raw: unknown): unknown {
  return (
    deepFindByKeys(raw, ["fiveElements", "wuxing", "五行"]) ??
    "（暂未提取到结构化五行信息，可参考 raw_markdown）"
  );
}

function normalizeTenGods(raw: unknown): unknown {
  return (
    deepFindByKeys(raw, ["tenGods", "shiShen", "shishen", "十神"]) ??
    "（暂未提取到结构化十神信息，可参考 raw_markdown）"
  );
}

function normalizeLuckCycles(raw: unknown): unknown {
  return (
    deepFindByKeys(raw, ["daYun", "dayun", "luckCycles", "bigLuck", "大运"]) ??
...
async function calcRawBazi(args: {
  ...
  gender: BaziGender;
  sect: 1 | 2;
}): Promise<{ raw: unknown; markdown: string }> {
  let buildBaziFromSolar: ((input: unknown) => unknown) | undefined;
  let buildBaziFromLunar: ((input: unknown) => unknown) | undefined;
  let baziToMarkdown: ((input: unknown) => string) | undefined;
  try {
    const pkg = (await import("cantian-tymext")) as UnknownRecord;
    buildBaziFromSolar = pkg.buildBaziFromSolar as (input: unknown) => unknown;
    buildBaziFromLunar = pkg.buildBaziFromLunar as (input: unknown) => unknown;
    baziToMarkdown = pkg.baziToMarkdown as (input: unknown) => string;
  } catch {
    throw new Error(
      [
        "排盘依赖未就绪，当前无法完成八字计算。",
...
export async function buildChart(input: BaziCalcInput): Promise<BaziChart> {
  ...
  const shouldApplyTrueSolar =
    trueSolarMode === "on" || (trueSolarMode === "auto" && Number.isFinite(longitude));
  ...
  const accuracyMode: AccuracyMode = input.time
    ? "full_chart"
  ...
  if (!input.time) {
    chart.notes.push(
      "未提供出生时间，已采用 12:00 进行内部计算，并按六柱精简模式输出。",
    );
...
async function main(): Promise<void> {
  ...
  const gender: BaziGender =
    genderArg === "male" || genderArg === "男"
      ? "male"
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().catch((error) => {
    const fallback = safeJsonParse<{ message?: string }>(
