...
const localePack: Record<SupportedLanguage, {
  ...
  noRecentFacts: string;
  noAdjustments: string;
  conversationMemory: string;
  noConversationYet1: string;
  noConversationYet2: string;
  analysisMode: string;
  analysisHint1: string;
  analysisHint2: string;
  analysisHint3: (axis: string) => string;
  languageMode: string;
  languageHint1: string;
  languageHint2: string;
  languageHint3: string;
  generatedDescription: (name: string, relation?: string) => string;
  formatCalendarType: (value: "solar" | "lunar") => string;
  displayGender: (value: "男" | "女") => string;
...
function t(text: string, language: SupportedLanguage): string {
  if (language === "zh") {
    return text;
  }
  if (language === "en") {
...
function formatCalendarType(value: "solar" | "lunar"): string {
...
function sanitizeDisplayedBaziText(text: string): string {
  return text
...
function labelLine(label: string, value: string, language: SupportedLanguage): string {
  const separator = language === "zh" || language === "ja" ? "：" : ": ";
  return `- ${label}${separator}${value}`;
}

function joinPair(first: string, second: string, language: SupportedLanguage): string {
...
function referenceValue(text: string, ...labels: string[]): string {
...
function profileReference(record: PersonaRecord, ...labels: string[]): string {
  return referenceValue(record.snapshot.reference_profile, ...labels);
}

function stateReference(record: PersonaRecord, ...labels: string[]): string {
...
function pickCurrentLuck(chart: BaziChart): string {
  ...
  const root = chart.luck_cycles as { 大运?: Array<Record<string, unknown>> } | undefined;
  const items = root?.大运 ?? [];
...
function extractRawBazi(record: PersonaRecord): UnknownRecord | undefined {
  return toRecord(record.chart.raw_bazi);
}

function collectHiddenTenGods(value: unknown): string[] {
...
function buildPillarDetail(rawBazi: UnknownRecord | undefined, key: "年柱" | "月柱" | "日柱" | "时柱"): string | undefined {
...
function buildProfileSection(record: PersonaRecord): string[] {
...
function buildBaziSection(record: PersonaRecord): string[] {
  const language = resolveLanguage("", record.preferences.preferred_language);
  const pack = localePack[language];
  const rawBazi = extractRawBazi(record);
...
function buildEntrySection(record: PersonaRecord): string[] {
  const language = resolveLanguage("", record.preferences.preferred_language);
  const pack = localePack[language];
...
function buildLuckTimelineSection(record: PersonaRecord): string[] {
  const language = resolveLanguage("", record.preferences.preferred_language);
  const pack = localePack[language];
  const cyclesRoot = record.chart.luck_cycles as { 起运日期?: unknown; 起运年龄?: unknown; 大运?: Array<Record<string, unknown>> } | undefined;
...
function categorizeMemory(memory: PersonaMemoryEntry[]): {
  facts: string[];
  adjustments: string[];
} {
  return {
...
function buildMemorySection(record: PersonaRecord): string[] {
  const language = resolveLanguage("", record.preferences.preferred_language);
  const pack = localePack[language];
...
function buildConversationSection(language: SupportedLanguage): string[] {
  const pack = localePack[language];
  return [
...
function buildAnalysisHintSection(record: PersonaRecord): string[] {
  const language = resolveLanguage("", record.preferences.preferred_language);
  const pack = localePack[language];
  return [
    pack.analysisMode,
...
function buildLanguageHintSection(language: SupportedLanguage): string[] {
  const pack = localePack[language];
  return [
    pack.languageMode,
    pack.languageHint1,
...
function buildAnalysisReply(record: PersonaRecord): string {
  const language = resolveLanguage("", record.preferences.preferred_language);
  const evidence = record.snapshot.evidence;
  if (language === "en") {
    return [
...
function selectNormalReferences(record: PersonaRecord, message: string): string[] {
...
function buildNormalReply(record: PersonaRecord, message: string): string {
...
export function respondInPersona(record: PersonaRecord, message: string, language?: SupportedLanguage): string {
