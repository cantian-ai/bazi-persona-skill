...
import { calendar_tool, bazi_chart_tool, bazi_flow_tool, chat_import_tool, memory_tool, persona_data_tool, type ToolRuntimeContext } from "./core-tools.js";
...
import type { PersonaConversationEntry, SupportedLanguage } from "./types.js";

function parseInitialFacts(args: Record<string, string>, input: string): string[] {
...
function resolveConversationPersona(args: Record<string, string>, input: string, baseDir: string): string | undefined {
...
function conversationEntry(params: {
  ...
  sessionId: string;
  source_type?: PersonaConversationEntry["source_type"];
...
function promptBody(context: ToolRuntimeContext, id: string): string {
...
function isStartupInjectionRequest(input: string): boolean {
...
export async function workflow_create_persona(params: {
  ...
  const chartResult = await bazi_chart_tool({
    name: args.name,
    gender: args.gender as "male" | "female" | "男" | "女",
...
export async function workflow_chat(params: {
  ...
  const mode: PersonaConversationEntry["mode"] = record.preferences.analysis_mode === "cheatsheet" ? "analysis" : "chat";
...
export async function workflow_import_chat(params: {
  ...
  const sourceType = (params.args["source-type"] ?? "text") as "text" | "json" | "ocr_text";
