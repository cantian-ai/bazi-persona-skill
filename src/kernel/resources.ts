...
import type { PersonaKnowledge, PromptManifest, PromptPack } from "./types.js";
...
let personaKnowledgeCache: PersonaKnowledge | undefined;

export function loadPromptPack(promptsDir = PROMPTS_DIR): PromptPack {
  const manifestPath = path.join(promptsDir, "manifest.json");
  if (!fileExists(manifestPath)) {
    throw new Error(`Prompt manifest missing: ${manifestPath}`);
  }

  const manifest = readJson<PromptManifest>(manifestPath);
...
function resolveKnowledgeFile(target: string): string {
...
function extractNamedJsonBlock(markdown: string, blockName: string): string {
...
export function loadPersonaKnowledge(knowledgeSource = PROMPT_KNOWLEDGE_FILE): PersonaKnowledge {
  ...
  const knowledge = JSON.parse(extractNamedJsonBlock(raw, "persona-knowledge")) as PersonaKnowledge;
  if (
