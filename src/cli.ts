...
export function inspectPersona(args: Record<string, string>): string {
...
export function deleteStoredPersona(args: Record<string, string>): string {
  const baseDir = resolvePersonaBaseDir(args["base-dir"]);
  const personas = listPersonas(baseDir);
  const input = args.input ?? args.slug ?? "";
  const ref = args.slug ? personas.find((persona) => persona.slug === args.slug) : resolvePersonaRef(input, personas);
...
const cliTextPack: Record<SupportedLanguage, {
  title: string;
  brand: string;
  intro1: string;
  intro2: string;
  ...
  fileExamples: string;
  chatExamples: string;
  unsupported: (action: string) => string;
...
function resolveCliLanguage(args: Record<string, string>, action?: string): SupportedLanguage {
...
export function renderCliHelp(language: SupportedLanguage = "zh"): string {
...
function normalizeCliAction(action?: string): "help" | "inspect" | "delete" | "unsupported" {
...
function renderUnsupportedAction(action: string | undefined, language: SupportedLanguage = "zh"): string {
  const pack = cliTextPack[language];
  return [
