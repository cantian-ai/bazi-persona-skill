...
export function resolveLanguage(input: string, preferred: "auto" | SupportedLanguage = "auto"): SupportedLanguage {
  return detectLanguage(input) ?? (preferred === "auto" ? "zh" : preferred);
}

export function parseArgsWithPositionalInput(argv: string[]): Record<string, string> {
...
export function routeIntent(input: string): RoutedIntent {
  const text = input.trim();
...
function extractLocation(text: string): string | undefined {
  ...
  return pieces.find((piece) =>
    !/\d{4}|男|女|male|female|创建|生成|更新|create|generate|build|persona|bazi|同事|朋友|伴侣|家人|客户|friend|partner|family|coworker|最近/u.test(piece),
  )?.replace(/人$/u, "");
}
...
function extractGender(text: string): "男" | "女" | undefined {
...
export function hydrateCreateArgs(args: Record<string, string>, input: string): Record<string, string> {
  const next = { ...args };
...
export function resolvePersonaRef(input: string, personas: PersonaRef[]): PersonaRef | undefined {
  const lower = input.toLowerCase();
  return personas.find((persona) =>
    lower.includes(persona.slug.toLowerCase()) || lower.includes(persona.name.toLowerCase()),
  );
}

export function extractUpdateFact(input: string): string {
