...
function normalizeGender(value: BaziChartToolInput["gender"]): {
  runtime: BaziGender;
} {
...
function normalizeMemoryEntry(entry: Partial<PersonaMemoryEntry>): PersonaMemoryEntry | undefined {
...
function normalizeMemoryList(entries: Array<Partial<PersonaMemoryEntry>>): PersonaMemoryEntry[] {
  return entries
    .map((entry) => normalizeMemoryEntry(entry))
    .filter((entry): entry is PersonaMemoryEntry => Boolean(entry));
}

function refreshRecord(record: PersonaRecord, context: ToolRuntimeContext): PersonaRecord {
...
function buildTransientRecordFromChart(input: BaziFlowToolInput): PersonaRecord {
  if (!input.chart) {
...
function normalizeConversationCandidate(
  raw: unknown,
  sourceType: ChatImportToolInput["source_type"],
): PersonaConversationEntry | undefined {
...
export async function bazi_chart_tool(input: BaziChartToolInput): Promise<{
...
export async function calendar_tool(input: CalendarToolInput): Promise<{
  at: string;
...
export async function bazi_flow_tool(
  input: BaziFlowToolInput,
...
export async function chat_import_tool(input: ChatImportToolInput): Promise<{
  ...
  const candidates = rawItems
    ...
    .filter((item): item is PersonaConversationEntry => Boolean(item))
...
export async function persona_data_tool(
  input: PersonaDataToolInput,
  context: ToolRuntimeContext,
): Promise<unknown> {
  ...
  if (action === "patch") {
    ...
    const nextRecord: PersonaRecord = {
      ...record,
...
function upsertByIdentity(existing: PersonaMemoryEntry[], incoming: PersonaMemoryEntry[]): {
  next: PersonaMemoryEntry[];
  created: number;
  updated: number;
} {
  let created = 0;
  ...
  for (const item of incoming) {
    const index = next.findIndex((current) => (
      (item.memory_id && current.memory_id && item.memory_id === current.memory_id) ||
...
export async function memory_tool(
  ...
  if (action === "delete") {
    ...
    const nextMemory = record.memory.filter((entry) => {
      ...
      return !filters.some((filter) =>
        (filter.memory_id && entry.memory_id === filter.memory_id) ||
  ...
  let nextMemory: PersonaMemoryEntry[] = record.memory;
  let created = 0;
  let updated = 0;
